...
namespace graphene { namespace chain {
   ...
   *
   */
  struct change_fee_for_operation_operation : public base_operation
  {
    ...
    change_fee_for_operation_operation() = default;
    explicit change_fee_for_operation_operation(account_id_type issuer, uint64_t new_fee, unsigned op_num, const string& comment)
        : issuer(issuer)
        , new_fee(new_fee)
        , op_num(op_num)
        , comment(comment) {}
...
FC_REFLECT( graphene::chain::change_fee_for_operation_operation::fee_parameters_type, )
FC_REFLECT( graphene::chain::change_fee_for_operation_operation, (fee)(issuer)(new_fee)(op_num)(comment) )
